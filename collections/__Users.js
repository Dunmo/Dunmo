...
var helpers = {
  ...
  _splitTasksByMaxTaskInterval: function (tasks) {
    ...
    if(!maxTaskInterval || maxTaskInterval == 0) return tasks;
    ...
    tasks.forEach(function (task) {
      ...
      if(task.remaining > 0) splitTasks.push(task);
    });
  ...
  todoList: function(freetimes) {
    ...
    if(isByDay) {
      freetimes = this.freetimesByDay(freetimes);
      console.log('freetimesByDay: ', freetimes);
    }
  ...
  _generateTodoList: function(freetimes, todos, algorithm, isByDay) {
    ...
    function fillFreetime(freetime) {
      if(todos.length > 0) {
    ...
    if(isByDay) {
      ...
      lodash.forOwn(freetimesByDay, function (freetimes, dayEnd, freetimesByDay) {
        dayEnd = Number(dayEnd);

        // Reset times per task
        todos.forEach(function (todo) {
          taskTimeAssignedToday[todo._id] = 0;
        });

        var list = freetimes.map(fillFreetime);

        todoList.concat(list);
      });
    }
    else {
      todoList = freetimes.map(fillFreetime);
    }
