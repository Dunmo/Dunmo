...
Natural = {
  parseImportance: function(str) {
    ...
    if(matches) {
        ...
        match = str.match(/(not|not very|not really|hardly|barely) important | (unimportant)/);
        if(match) return 1;
        match = str.match(/(somewhat|sort of|kind of|a bit|a little) important/);
        if(match) return 2;
        match = str.match(/(very|really|extremely|super) important/);
        if(match) return 3;
    }
  },

  parseDueAt: function(str) {
    var match = str.match(/(?:due)* (?:by|before|on|at) (.*) for | (?:due)* (?:by|before|on|at) (.*)/);
    match = match[0];
  ...
  parseTitle: function (str) {
    ...
    var imp = parseImportance(str);
    var due = parseDueAt(str);
    var duration = parseDuration(str);
    str.replace(imp, '');
    str.replace(due, '');
  ...
  parseTask: function (str) {
    ...
    var due = Natural.parseDueAt(str);
    var duration = Natural.parseDuration(str);
    var title = Natural.parseTitle(str);
