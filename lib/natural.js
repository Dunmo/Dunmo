...
Natural = {
  ...
  parseDueAt: function(str) {
    ...
    if(match) {
      ...
      if ( date.isValid() ) return [str.replace(match, '').slimNTidy(), date];
    }
    ...
    if ( date.isValid() ) return [str.slimNTidy(), date];
    ...
    for (var i = arr.length; i > 0; i--) {
      for (var j = 0; j < i; j++) {
        ...
        if ( date.isValid() ) {
          ...
          ret = ret.replace(/due before | due before|due at | due at|due by | due by|due on | due on|due | due/g, '');
          return [ret.slimNTidy(), date];
        }
      };
    };
  },
