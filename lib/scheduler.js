...
Scheduler = {
  ...
  _fillFreetime: function (freetime, todos) {
    ...
    while(remaining > 0 && todos.length > 0) {
      ...
      console.log('todos: ', todos);
      console.log('snoozedTodos: ', snoozedTodos);

      todoIndex = 0;

      // Don't schedule the same task twice in a row
      if(_.last(freetime.todos) && todos[todoIndex]._id === _.last(freetime.todos)._id) {
        // get the next task
        if(todos.length > todoIndex+1) {
          todoIndex++;
